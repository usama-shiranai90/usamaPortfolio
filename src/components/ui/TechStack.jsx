        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {Object.entries(skillsToRender).map(([category, { icon: Icon, items }], index) => (
                <motion.div
                    ...
                    viewport={VIEWPORT}
                    transition={{ duration: DURATION.base, ease: EASE, delay: index * 0.1 }}
                    className="bg-theme-card/20 backdrop-blur-sm border border-theme-border rounded-xl p-6 hover:bg-theme-card/30 transition-colors group"
                >
                    ...
                    <div className="flex flex-wrap gap-2">
                        {items.map((item) => (
                            <Tag
                                key={item}
                                className="cursor-default hover:text-cyan-accent hover:ring-cyan-accent/40"
                            >
                                {item}
                            </Tag>
                        ))}
