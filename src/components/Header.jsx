...
import avatarImage from '../assets/images/owl-shashin.jpg';
...
function MobileNavItem({ href, children }) {
  return (
      <li>
        <PopoverButton href={href} className="block py-2">
          {children}
        </PopoverButton>
      </li>
  );
...
function ThemeToggle() {
    // Manage theme using React's useState hook
    const [theme, setTheme] = useState('dark');
    const [mounted, setMounted] = useState(false);

    // Check and set the initial theme based on localStorage or system preference
    useEffect(() => {
    ...
    return (
        ...
        >
            {theme === 'light' ? (
                ...
                <MoonIcon className="h-6 w-6 fill-zinc-700 stroke-zinc-500 transition group-hover:stroke-zinc-400" />
            )}
        </button>
    );
}
